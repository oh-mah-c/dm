...
blocks = r'\b(in|out|t|logits|z|mu|logvar|img|patches|resized|labels|grads|inputs|outputs)\b'
...
for filepath in h_files + c_files:
    ...
    for line in lines:
        ...
        # 6. Dimensions access - ONLY ON KNOWN BLOCKS
        text = re.sub(blocks + r'->n\b', r'DM_NCHW_N(\1)', text)
        text = re.sub(blocks + r'->c\b', r'DM_NCHW_C(\1)', text)
        text = re.sub(blocks + r'->h\b', r'DM_NCHW_H(\1)', text)
        text = re.sub(blocks + r'->w\b', r'DM_NCHW_W(\1)', text)
        ...
        # 7. Data access
        text = re.sub(blocks + r'\.data\[', r'((float*)\1.data)[', text)
        text = re.sub(blocks + r'->data\[', r'((float*)\1->data)[', text)
...
print("Safely refactored specific blocks!")
