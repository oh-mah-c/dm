...
int dm_resnet_basic_block(const DM_Block *in, DM_Block *out, int out_c, int stride, unsigned int seed) {
    ...
    // Main path: Conv1 (3x3)
    float *w1 = make_weights((size_t)out_c * DM_NCHW_C(in) * 3 * 3, seed + 1, 0.08f);
    if (!w1) return -1;
    ...
    if (!w2) { dm_block_free(&x1); return -1; }
    ...
    if (stride != 1 || DM_NCHW_C(in) != out_c) {
        // Conv1x1 shortcut
        float *w_short = make_weights((size_t)out_c * DM_NCHW_C(in) * 1 * 1, seed + 11, 0.08f);
        if (!w_short) { dm_block_free(&x2); return -1; }
        ...
        if (!gamma_s || !beta_s || !mean_s || !var_s) {
            ...
            dm_block_free(&x2); dm_block_free(&shortcut); return -1;
        }
...
int dm_resnet18_forward(const DM_Block *input, DM_Block *logits, int classes, unsigned int seed) {
    ...
    float *w_conv1 = make_weights((size_t)64 * DM_NCHW_C(input) * 7 * 7, s++, 0.08f);
    if (!w_conv1) return -1;
    ...
    if (!g1 || !b1 || !m1 || !v1) {
        ...
        dm_block_free(&t1); return -1;
    }
    rc = dm_batch_norm(&t1, g1, b1, m1, v1, 1e-5f);
    free(g1); free(b1); free(m1); free(v1);
    if (rc != 0) { dm_block_free(&t1); return -1; }
