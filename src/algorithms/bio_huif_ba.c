...
static DM_Status run(DM_Dataset *ds, void *params) {
    ...
    Bat *pop = malloc(sizeof(Bat) * pop_size);
    double r0 = 0.5;
    ...
    for (int i = 0; i < pop_size; i++) {
        if (pop[i].fitness > gbest_fitness) { gbest_fitness = pop[i].fitness; memcpy(gbest, pop[i].bits, htwui_count); }
    }

    int *diff_indices = malloc(sizeof(int) * htwui_count);
    uint8_t *local_bits = malloc(htwui_count);
    double alpha = 0.9;
    double gamma = 0.9;
    ...
    for (int iter = 0; iter < max_iter; iter++) {
        for (int i = 0; i < pop_size; i++) {
            // Update according to Algorithm 8
            // 1. Randomly change one bit
            int r_idx = rand() % htwui_count;
            pop[i].bits[r_idx] = 1 - pop[i].bits[r_idx];

            // 2. Frequency update
            double f_min = 0, f_max = 2;
            pop[i].frequency = f_min + (f_max - f_min) * ((double)rand() / RAND_MAX);

            // 3. Diff with gbest and velocity update
            size_t diff_len = get_bit_diff(pop[i].bits, gbest, diff_indices);
            if (diff_len > 0) {
                int v_i2 = (int)((double)diff_len * (pop[i].frequency / f_max));
            ...
            // 4. Local Search
            if (((double)rand() / RAND_MAX) > pop[i].pulse_rate) {
                memcpy(local_bits, pop[i].bits, htwui_count);
                int m_idx = rand() % htwui_count;
                local_bits[m_idx] = 1 - local_bits[m_idx];
                pev_check(local_bits);
                double local_u = calculate_utility(local_bits);
                if (local_u >= total_min_util) add_to_shui(local_bits, local_u);
                
                // Acceptance based on loudness
                if (((double)rand() / RAND_MAX) < pop[i].loudness && local_u > pop[i].fitness) {
                    memcpy(pop[i].bits, local_bits, htwui_count);
                    pop[i].fitness = local_u;
                    // Update loudness and pulse rate
                    pop[i].loudness *= alpha;
                    pop[i].pulse_rate = r0 * (1 - exp(-gamma * (iter + 1)));
                }
            } else {
                pop[i].fitness = calculate_utility(pop[i].bits);
                if (pop[i].fitness >= total_min_util) add_to_shui(pop[i].bits, pop[i].fitness);
        ...
        // Roulette Wheel Selection for gbest of next population
        roulette_wheel_init_gbest(gbest);
        gbest_fitness = calculate_utility(gbest);
    }

    dm_bench_record_results(ctx.shui_count, 0);

    // Cleanup
    free(local_bits); free(diff_indices); free(gbest);
...
DM_Algorithm bio_huif_ba_algo = {
    .id = "bio_huif_ba",
    .name = "Bio-HUIF-BA",
    .description = "Bio-inspired HUIF based on Bat Algorithm (Song & Huang 2018) with Diverse Optimal Value Framework.",
