...
static DM_Status run(DM_Dataset *ds, void *params) {
    ...
    DM_HUIM_SA_Params *p = (DM_HUIM_SA_Params *)params;
    double min_util_ratio = p ? p->min_utility : 0.01;
    double temp = p ? p->temp : 100000.0;
    double min_temp = p ? p->min_temp : 0.00001;
    double alpha = p ? p->alpha : 0.9993;
    ...
    htwuis = malloc(sizeof(SA_Item) * htwui_count);
    size_t h_idx = 0;
    for (uint32_t i = 0; i <= ds->max_id; i++) {
    ...
    double twu_sum_init = 0;
    for (size_t i = 0; i < htwui_count; i++) twu_sum_init += htwuis[i].twu;

    for (int i = 0; i < pop_size; i++) {
        pop[i].bits = calloc(htwui_count, 1);
        pop[i].ts = dm_bitset_create(ds->count);
        int num_ones = (rand() % 5) + 1;
        for (int j = 0; j < num_ones; j++) {
            double r = ((double)rand() / RAND_MAX) * twu_sum_init;
            double cur = 0;
    ...
    while (temp > min_temp) {
        for (int i = 0; i < pop_size; i++) {
            bool is_hui = (pop[i].fitness >= total_min_util);
            double ar = exp(temp / (1.0 + temp));
            double rv = ((double)rand() / RAND_MAX) * 10.0;
        ...
        for (int i = 0; i < pop_size; i++) {
            ...
            if (pevc(neighbor_bits, neighbor_ts, src)) {
                ...
                if (neighbor_fitness >= pop[i].fitness) {
                    ...
                    double delta = pop[i].fitness - neighbor_fitness;
                    double p_accept = exp(-delta / temp);
...
static DM_Algorithm huim_sa_algo = {
    .id = "huim_sa",
    .name = "HUIM-SA",
    .description = "Mining High-Utility Itemsets with Simulated Annealing (Nawaz et al. 2021).",
    .supported_types = (1 << DM_TYPE_UTILITY),
    .run = run
};

DM_REGISTER_ALGORITHM(huim_sa_algo)
