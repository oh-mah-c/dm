...
static void tqcs_add_ordered(TQCS *tqcs, FQItem a, FQItem b, int oa, int ob, double twu, double utility) {
    ...
    if (tqcs->entries[slot].key == key) {
        ...
        tqcs->entries[slot].utility += utility;
        return;
    }
...
static int cmp_ul_utility_asc(const void *a, const void *b) {
    FUL *x = *(FUL **)a;
    FUL *y = *(FUL **)b;
    ...
    if (x->count > y->count) return 1;
    for (size_t i = 0; i < x->count && i < y->count; i++) {
        if (x->qitems[i].item != y->qitems[i].item) return (x->qitems[i].item < y->qitems[i].item) ? -1 : 1;
        if (x->qitems[i].low != y->qitems[i].low) return x->qitems[i].low - y->qitems[i].low;
        if (x->qitems[i].high != y->qitems[i].high) return x->qitems[i].high - y->qitems[i].high;
    }
    return 0;
}
...
static void topk_init(FTopKQueue *queue, size_t k) {
    queue->lists = NULL;
    queue->count = 0;
    queue->capacity = 0;
    queue->k = k;
...
static void topk_free(FTopKQueue *queue) {
    for (size_t i = 0; i < queue->count; i++) free_ul(queue->lists[i]);
    free(queue->lists);
}

static bool same_qitemset(FUL *a, FUL *b) {
    if (a->count != b->count) return false;
    for (size_t i = 0; i < a->count; i++) {
        if (!qitem_equal(a->qitems[i], b->qitems[i])) return false;
    }
    return true;
}

static bool topk_insert(FTopKQueue *queue, FUL *itemset, double *min_utility) {
    if (queue->k == 0) return true;
    for (size_t i = 0; i < queue->count; i++) {
        if (same_qitemset(queue->lists[i], itemset)) return true;
    }
    if (queue->count == queue->capacity) {
        size_t new_capacity = queue->capacity ? queue->capacity * 2 : 16;
        FUL **new_lists = realloc(queue->lists, sizeof(FUL *) * new_capacity);
        if (!new_lists) return false;
        queue->lists = new_lists;
        queue->capacity = new_capacity;
    }
    FUL *copy = clone_ul(itemset);
    if (!copy) return false;
    queue->lists[queue->count++] = copy;
    qsort(queue->lists, queue->count, sizeof(FUL *), cmp_ul_utility_asc);

    if (queue->count > queue->k) {
        *min_utility = queue->lists[queue->count - queue->k]->sum_eutil;
        size_t out = 0;
        for (size_t i = 0; i < queue->count; i++) {
            if (queue->lists[i]->sum_eutil + 1e-9 < *min_utility) {
                free_ul(queue->lists[i]);
            } else {
                queue->lists[out++] = queue->lists[i];
            }
        }
        queue->count = out;
    }
...
static void raise_minutil_from_riu(FExact *items, size_t count, size_t k, double *min_utility) {
    if (k == 0 || count < k) return;
    double *values = malloc(sizeof(double) * count);
    if (!values) return;
    for (size_t i = 0; i < count; i++) values[i] = items[i].utility;
    qsort(values, count, sizeof(double), cmp_double_desc);
    if (values[k - 1] > *min_utility) *min_utility = values[k - 1];
    free(values);
}

static void raise_minutil_from_cud(TQCS *tqcs, size_t k, double *min_utility) {
    if (k == 0 || tqcs->count < k) return;
    double *values = malloc(sizeof(double) * tqcs->count);
    if (!values) return;
    size_t count = 0;
    for (size_t i = 0; i < tqcs->capacity; i++) {
        if (tqcs->entries[i].key != 0) values[count++] = tqcs->entries[i].utility;
    }
    if (count >= k) {
        qsort(values, count, sizeof(double), cmp_double_desc);
        if (values[k - 1] > *min_utility) *min_utility = values[k - 1];
...
static void collect_status_tkq(FUL *z, double *min_utility, double qrc, FSet *h, FSet *e, FSet *c,
                               FTopKQueue *queue) {
    if (z->sum_eutil >= *min_utility) {
        topk_insert(queue, z, min_utility);
        set_push_unique(h, z);
    } else {
...
static void recursive_search_tkq(FUL *prefix, FUL **qis, size_t qis_count, FUL **pstar, size_t pstar_count,
                                 double *min_utility, double qrc, DM_FHUQI_Combine_Method method,
                                 size_t k, FTopKQueue *queue) {
    (void)k;
    for (size_t i = 0; i < qis_count; i++) {
        ...
        for (size_t j = 0; j < pstar_count; j++) {
            ...
            collect_status_tkq(z, min_utility, qrc, &h, &e, &c, queue);
        }

        combine_candidates(c.lists, c.count, *min_utility, qrc, method, &hr);
        for (size_t r = 0; r < hr.count; r++) {
            topk_insert(queue, hr.lists[r], min_utility);
        }

        for (size_t n = 0; n < h.count; n++) set_push_unique(&next_qis, h.lists[n]);
        for (size_t n = 0; n < e.count; n++) set_push_unique(&next_qis, e.lists[n]);
        for (size_t n = 0; n < hr.count; n++) set_push_unique(&next_qis, hr.lists[n]);
        if (next_qis.count > 1) qsort(next_qis.lists, next_qis.count, sizeof(FUL *), cmp_list_order);

        if (next_qis.count > 0 && pstar_next.count > 0) {
            recursive_search_tkq(px, next_qis.lists, next_qis.count, pstar_next.lists, pstar_next.count,
                                 min_utility, qrc, method, k, queue);
        }
...
static DM_Status run_tkq(DM_Dataset *ds, void *params) {
    if (ds->type != DM_TYPE_QUANTITY) return DM_ERROR_INCOMPATIBLE;

    DM_TKQ_Params *tp = (DM_TKQ_Params *)params;
    size_t k = (tp && tp->k > 0) ? tp->k : 10;
    double qrc = (tp && tp->qrc > 0.0) ? tp->qrc : 3.0;
    DM_FHUQI_Combine_Method method = tp ? tp->combine_method : DM_FHUQI_COMBINE_ALL;
    if (!tp || !tp->profit_path || tp->profit_path[0] == '\0') return DM_ERROR_INVALID_PARAM;
    ...
    if (!load_profit_table(tp->profit_path, profits, ds->max_id + 1)) {
        free(profits);
        return DM_ERROR_IO;
    }
    ...
    for (size_t i = 0; i < ds->count; i++) {
        for (size_t j = 0; j < data[i].count; j++) {
            ...
            double utility = profits[data[i].items[j].id] * data[i].items[j].quantity;
            if (idx < 0) {
                ...
                all[all_count].utility = utility;
                all_count++;
            } else {
                all[idx].twu += g_transaction_utilities[i];
                all[idx].utility += utility;
            }
        }
    }

    double min_utility = 0.0;
    raise_minutil_from_riu(all, all_count, k, &min_utility);
    ...
    for (size_t i = 0; i < promising_count; i++) {
        ...
        if (!initial[i]) {
            for (size_t n = 0; n < i; n++) free_ul(initial[n]);
            free(initial);
            free(promising);
            free(g_rank_index);
            g_rank_index = NULL;
        ...
        for (size_t a = 0; a < count; a++) {
            for (size_t b = a + 1; b < count; b++) {
                tqcs_add_ordered(&g_tqcs, transaction[a].qitem, transaction[b].qitem,
                                 ...
                                 transaction[a].eutil + transaction[b].eutil);
            }
        }
    ...
    raise_minutil_from_cud(&g_tqcs, k, &min_utility);
    ...
    for (size_t i = 0; i < promising_count; i++) {
        collect_status_tkq(initial[i], &min_utility, qrc, &h, &e, &c, &queue);
    }

    combine_candidates(c.lists, c.count, min_utility, qrc, method, &hr);
    for (size_t i = 0; i < hr.count; i++) {
        topk_insert(&queue, hr.lists[i], &min_utility);
    }
    ...
    printf("[TKQ] Starting search with k %zu, internal minutil %.3f, qrc %.3f, method %d...\n",
           k, min_utility, qrc, (int)method);
    recursive_search_tkq(NULL, qis.lists, qis.count, initial, promising_count, &min_utility, qrc, method, k, &queue);
    printf("[TKQ] Found %zu top-k Quantitative High Utility Itemsets. Final minutil %.3f.\n",
           queue.count, min_utility);

    total_qitems_sum = 0;
    for (size_t i = 0; i < queue.count; i++) total_qitems_sum += queue.lists[i]->count;
    ...
    free(promising);
    tqcs_free(&g_tqcs);
    dm_bench_record_results(queue.count, total_qitems_sum);
    topk_free(&queue);
    return DM_SUCCESS;
}

static DM_Algorithm tkq_algo = {
    .id = "tkq",
    .name = "TKQ",
    .description = "Top-K Quantitative High Utility Itemset Miner with RIU, CUD, and Update Queue raising.",
    .supported_types = (1 << DM_TYPE_QUANTITY),
    .run = run_tkq
};

DM_REGISTER_ALGORITHM(tkq_algo)
