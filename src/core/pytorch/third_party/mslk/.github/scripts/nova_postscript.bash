...
cd "${MSLK_REPO}" || { echo "[NOVA] Failed to cd to ${MSLK_REPO} from $(pwd)"; };
test_all_mslk_modules "${BUILD_ENV_NAME}"
end_time=$(date +%s)
runtime=$((end_time-start_time))
start_time=${end_time}
...
chown -R 1000:1000 /__w/MSLK/MSLK/ || echo "Unable to chown 1000:1000 from $USER, uid: $(id -u)"
