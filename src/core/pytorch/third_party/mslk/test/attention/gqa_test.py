...
class Int4GQATest(unittest.TestCase):
    ...
    def test_gqa(
        ...
        """
        Test correctness of torch.ops.mslk.gqa_attn_splitk against the
        ...
        for split_k in [1, 2, 4, 8, 13, 16]:
            z, _, _ = torch.ops.mslk.gqa_attn_splitk(
                q,
                cache_k,
                cache_v,
                seq_positions,
    ...
    def test_mqa_main(  # noqa C901
        ...
        if dtype in ["fp8", "int4"]:
            ...
            if dtype == "fp8":
                cache_k_ref, cache_v_ref = torch.ops.mslk.dequantize_fp8_cache(
                ...
                cache_k_ref, cache_v_ref = torch.ops.mslk.dequantize_int4_cache(
                    cache_k.view(torch.uint8).contiguous(),
                    cache_v.view(torch.uint8).contiguous(),
                    torch.tensor(kv_seqlens, device=cache_k.device, dtype=torch.int32),
                    num_groups=num_groups,
        ...
        if mqa:
            for split_k in [1, 2, 4, 8, 13, 16]:
                if not gqa_attn_splitk_known_failure:
                    z_split_k, _, _ = torch.ops.mslk.gqa_attn_splitk(
                        q,
                        cache_k,
                        cache_v,
                        seq_positions,
