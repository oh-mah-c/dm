...
using source_location = mslk::utils::source_location;
...
TEST(KernelLauncherTest, array_kernel_launch) {
  ...
  // Test <<< >>> kernel launch with raw cudaStream_t
  EXPECT_NO_THROW({
  ...
  // Test coooperative kernel launch
  EXPECT_NO_THROW({
    C = at::full(
        {size}, -1, at::TensorOptions().dtype(at::kFloat).device(at::kCUDA));
  ...
  // Test coooperative kernel launch with raw cudaStream_t
  EXPECT_NO_THROW({
    C = at::full(
        {size}, -1, at::TensorOptions().dtype(at::kFloat).device(at::kCUDA));
