#!/bin/bash
# Copyright (c) Meta Platforms, Inc. and affiliates.
# All rights reserved.
#
# This source code is licensed under the BSD-style license found in the
# LICENSE file in the root directory of this source tree.


# shellcheck disable=SC1091,SC2128
. "$( dirname -- "$BASH_SOURCE"; )/utils_base.bash"
# shellcheck disable=SC1091,SC2128
. "$( dirname -- "$BASH_SOURCE"; )/utils_pip.bash"
# shellcheck disable=SC1091,SC2128
. "$( dirname -- "$BASH_SOURCE"; )/utils_pytorch.bash"

################################################################################
# MSLK Integration Scripts
################################################################################

integration_setup_conda_environment () {
  local env_name="$1"
  local compiler="$2"
  local python_version="$3"
  local pytorch_channel_version="$4"
  local pytorch_variant_type_version="$5"
  if [ "$pytorch_variant_type_version" == "" ]; then
    echo "Usage: ${FUNCNAME[0]} ENV_NAME COMPILER PYTHON_VERSION PYTORCH_INSTALLER PYTORCH_CHANNEL[/VERSION] PYTORCH_VARIANT_TYPE/PYTORCH_VARIANT_VERSION"
    echo "Example(s):"
    echo "    ${FUNCNAME[0]} build_env clang 3.13 pip test/1.0.0 cuda/12.6.3       # Setup environment with pytorch-test 1.0.0 for Clang + Python 3.13 + CUDA 12.6.3"
    return 1
  else
    echo "################################################################################"
    echo "# Setup MSLK Build Environment (All Steps)"
    echo "#"
    echo "# [$(date --utc +%FT%T.%3NZ)] + ${FUNCNAME[0]} ${*}"
    echo "################################################################################"
    echo ""
  fi

  echo "[INTEGRATION] Creating the Build Environment: ${env_name} ..."
  create_conda_environment  "${env_name}" "${python_version}"           || return 1

  IFS='/' read -r pytorch_variant_type pytorch_variant_version <<< "${pytorch_variant_type_version}"
  echo "[INTEGRATION] Will install PyTorch (version, variant): (${pytorch_variant_type}, ${pytorch_variant_version})"

  if [[ "$pytorch_variant_type" == "cuda" ]]; then
    print_exec conda env config vars set -n "${env_name}" BUILD_CUDA_VERSION="${pytorch_variant_version}"
  elif [[ "$pytorch_variant_type" == "rocm" ]]; then
    print_exec conda env config vars set -n "${env_name}" BUILD_ROCM_VERSION="${pytorch_variant_version}"
  fi

  # Install C++ compiler and build tools (all MSLK variants)
  if [ "$compiler" == "gcc" ] || [ "$compiler" == "clang" ]; then
    install_cxx_compiler  "${env_name}" "${compiler}"   || return 1
  fi
  install_build_tools     "${env_name}"                 || return 1

  if [[ "$pytorch_variant_type" == "cuda" ]]; then
    # Install CUDA tools and runtime
    install_cuda            "${env_name}" "${pytorch_variant_version}"  || return 1
  elif [[ "$pytorch_variant_type" == "rocm" ]] && ! [[ "$(hostname)" =~ ^.*facebook.com$ ]]; then
    # Install ROCm tools and runtime
    install_rocm_ubuntu     "${env_name}" "${pytorch_variant_version}"  || return 1
  fi

  # Install PyTorch
  install_pytorch_pip       "${env_name}" "${pytorch_channel_version}" "${pytorch_variant_type_version}" || return 1

  echo "[INTEGRATION] Successfully created the Conda build environment: ${env_name}"
  export env_name="${env_name}"
}

integration_mslk_build_and_install () {
  local env_name="$1"
  local build_target_variant="$2"
  local repo="$3"
  if [ "$build_target_variant" == "" ]; then
    echo "Usage: ${FUNCNAME[0]} ENV_NAME BUILD_TARGET_VARIANT"
    echo "Example(s):"
    echo "    ${FUNCNAME[0]} build_env default/cuda   # Build and install MSLK for CUDA (All Steps)"
    return 1
  else
    echo "################################################################################"
    echo "# MSLK build + install Combo Step"
    echo "#"
    echo "# [$(date --utc +%FT%T.%3NZ)] + ${FUNCNAME[0]} ${*}"
    echo "################################################################################"
    echo ""
  fi

  if [ "$repo" == "" ]; then
    echo "[INTEGRATION]: repo argument not provided, defaulting to ~/MSLK"
    repo=~/MSLK
  fi

  # Start from the repo root directory
  pushd "${repo}"                                                       || return 1
  # Remove previous build artifacts
  rm -rf "${repo}/dist/"                                                || return 1
  # Build the MSLK package
  prepare_mslk_build    "${env_name}"                                   || return 1
  build_mslk_package    "${env_name}" nightly "${build_target_variant}" || return 1
  popd                                                                  || return 1

  # Move to another directory, to avoid Python package import confusion, since
  # there exists a mslk/ subdirectory in the MSLK repo
  mkdir -p _tmp_dir_mslk                                    || return 1
  pushd _tmp_dir_mslk                                       || return 1
  # Uninstall the MSLK package (if installed)
  uninstall_pip_wheel  "${env_name}" "mslk-"                || return 1
  # Install the MSLK package and test the package import
  install_mslk_wheel    "${env_name}" ${repo}/dist/*.whl    || return 1
  # Return to the repo root directory
  popd                                                      || return 1

  echo "[INTEGRATION] Successfully built and installed MSLK in the Conda environment: ${env_name}"
}

integration_mslk_build_and_install_and_test () {
  local env_name="$1"
  local build_target_variant="$2"
  local repo="$3"

  if [ "$repo" == "" ]; then
    echo "[INTEGRATION]: repo argument not provided, defaulting to ~/MSLK"
    repo=~/MSLK
  fi

  integration_mslk_build_and_install "${env_name}" "${build_target_variant}" "${repo}"  || return 1
  test_all_mslk_modules "${env_name}" "${repo}"                                         || return 1
}

integration_mslk_pip_install_matrix_run () {
  local variant_type="$1"
  local pytorch_channel_version="$2"
  local mslk_channel_version="$3"
  local repo="$4"
  if [ "$mslk_channel_version" == "" ]; then
    echo "Usage: ${FUNCNAME[0]} ENV_NAME PYTORCH_CHANNEL[/VERSION] MSLK_CHANNEL[/VERSION]"
    echo "Example(s):"
    echo "    ${FUNCNAME[0]} test_env cpu test/2.2.0 test/0.8.0     # Run tests against all Python versions with PyTorch test/2.2.0 and MSLK test/0.8.0 (CPU-only)"
    echo "    ${FUNCNAME[0]} test_env cuda test/2.3.0 test/0.8.0    # Run tests against all Python versions with PyTorch test/2.3.0 and MSLK test/0.8.0 (all CUDA versions)"
    return 1
  else
    echo "################################################################################"
    echo "# Run Bulk MSLK Testing from PIP Package Installation"
    echo "#   (Environment Setup + Download + Install + Test)"
    echo "#"
    echo "# [$(date --utc +%FT%T.%3NZ)] + ${FUNCNAME[0]} ${*}"
    echo "################################################################################"
    echo ""
  fi

  if [ "$repo" == "" ]; then
    echo "[INTEGRATION]: repo argument not provided, defaulting to ~/MSLK"
    repo=~/MSLK
  fi

  __single_run () {
    local py_version="$1"
    local variant_version="$2"
    local repo="$3"

    local env_name="test_py${py_version}_pytorch_${pytorch_channel_version}_mslk_${mslk_channel_version}_${variant_type}/${variant_version}"
    local env_name="${env_name//\//_}"
    integration_setup_conda_environment  "${env_name}" gcc "${py_version}" pip "${pytorch_channel_version}" "${variant_type}" "${variant_version}" || return 1
    install_mslk_pip        "${env_name}" "${mslk_channel_version}" "${variant_type}/${variant_version}"  || return 1
    pushd "${repo}"                                                                                          || return 1

    test_all_mslk_modules "${env_name}"
    local retcode=$?

    echo "################################################################################"
    echo "# RUN SUMMARY"
    echo "#"
    echo "# Conda Environment       : ${env_name}"
    echo "# Python Version          : ${py_version}"
    echo "# PyTorch Version         : ${pytorch_channel_version}"
    echo "# MSLK Version      : ${mslk_channel_version}"
    echo "# Variant type / Version  : ${variant_type}/${variant_version}"
    echo "#"
    echo "# Run Result              : $([ $retcode -eq 0 ] && echo "PASSED" || echo "FAILED")"
    echo "################################################################################"

    if [ $retcode -eq 0 ]; then
      # Clean out environment only if there were no errors
      conda remove -n "$env_name" -y --all
    fi

    popd || return 1
    return $retcode
  }

  local python_versions=(
    3.9
    3.10
    3.11
    3.12
    3.13
  )

  if [ "$variant_type" == "cuda" ]; then
    local variant_versions=(
      12.6.3
      12.8.1
      12.9.1
      13.0.0
    )
  elif [ "$variant_type" == "rocm" ]; then
    local variant_versions=(
      6.3
      6.4
    )
  elif [ "$variant_type" == "cpu" ]; then
    local variant_versions=(
      "none"
    )
  else
    echo "[TEST] Invalid variant type: ${variant_type}"
    return 1
  fi

  for py_ver in "${python_versions[@]}"; do
    for var_ver in "${variant_versions[@]}"; do
      __single_run "${py_ver}" "${var_ver}" "${repo}" || return 1
    done
  done
}
