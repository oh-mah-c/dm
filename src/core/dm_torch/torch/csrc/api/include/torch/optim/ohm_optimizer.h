...
struct TORCH_API OhmOptimizerParamState
    : public OptimizerCloneableParamState<OhmOptimizerParamState> {
 public:
  void serialize(torch::serialize::InputArchive& archive) override;
  void serialize(torch::serialize::OutputArchive& archive) const override;
  TORCH_API friend bool operator==(
      const OhmOptimizerParamState& lhs,
      const OhmOptimizerParamState& rhs);
};
